...
import type { FamilyOffer, SubscriptionPlan } from "./familyTypes";
...
const instanceFor = async (appUserId: string) => {
  ...
  if (!purchases) {
    purchases = Purchases.configure({
      apiKey: __REVENUECAT_PUBLIC_API_KEY__,
      appUserId,
      flags: {
  ...
  } else if (purchases.getAppUserId() !== appUserId) {
    await purchases.changeUser(appUserId);
  }
  return purchases;
};

export async function loadRevenueCatOffers(appUserId: string): Promise<FamilyOffer[]> {
