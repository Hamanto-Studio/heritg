...
const isExactRecord = (record, expected) => record.type === expected.type &&
  normalizedHost(record.content) === normalizedHost(expected.content) && record.proxied === false;
...
export async function configureStagingDns(token, fetchImpl = fetch) {
  ...
  const records = await apiRequest(
    fetchImpl,
    token,
    `/zones/${zoneId}/dns_records?name=${encodeURIComponent(STAGING_DNS.name)}`
  );
  if (records.length > 0) {
    if (records.length !== 1 || !isExactRecord(records[0], STAGING_DNS)) {
      throw new Error(`${STAGING_DNS.name} already has a conflicting DNS record; refusing to replace it.`);
    }
    return { action: "unchanged" };
  }

  await apiRequest(fetchImpl, token, `/zones/${zoneId}/dns_records`, {
    method: "POST",
    body: JSON.stringify({
...
const keychainToken = () => {
  ...
  try {
    ...
    throw new Error("No Cloudflare token found. Follow the secure Keychain command in docs/DEPLOYMENT.md.");
  }
};

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  configureStagingDns(keychainToken())
    .then(({ action }) => {
      process.stdout.write(
        `${STAGING_DNS.name} ${action === "created" ? "created" : "already configured"} as ` +
