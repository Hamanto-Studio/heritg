...
- Optionally sync the iOS database between the user's Apple devices through Core Data and their private iCloud/CloudKit database.
...
- iOS: Swift, SwiftUI, Core Data, and CloudKit.
- Android: Kotlin, Jetpack Compose, Room, and Android platform APIs.

Reasons:

- Native persistence is required: Core Data on iOS and Room on Android.
...
- iOS uses Core Data as its local source of truth.
- Core Data uses a CloudKit-enabled persistent container to sync through the user's private iCloud database when the user enables iCloud sync and is signed in to iCloud.
...
- Store media in each platform's app-private directory; store metadata and content hashes in Core Data or Room.
...
- Never expose Core Data object identifiers, Room row IDs, CloudKit record IDs, or platform file paths as portable identifiers.
...
- Core Data and Room are platform runtime stores, not transfer formats.
...
- Core Data and Room migration tests from every released schema
- Core Data/CloudKit tests for signed-out, offline, quota, conflict, and delayed-sync states
...
- Specified repository operations, Core Data models, Room entities, and migrations
...
- Device app lock
- Core Data/CloudKit sync reliability and failure-state testing
