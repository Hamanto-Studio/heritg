...
- Keep the encryption page's CSS and assets inside the GitHub Pages `docs/`
  tree so the route remains self-contained and does not depend on the retired
  `hamanto.com` hostname.
...
home for your family history`. The app response must report Vercel and retain
its security headers. The encryption response at
`https://family.heritg.us/blog/e2e-encryption/` must report GitHub Pages and the
title `How HERITG encrypts your family archive`. All responses and the
